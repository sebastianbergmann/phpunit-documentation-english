...
The main difference between assertContains() and assertContainsEquals() is identity check (== vs ===). While assertContains() ensures that the same instance exists in haystack, assertContainsEquals() only ensures that an instance with the same attribute values exist.
